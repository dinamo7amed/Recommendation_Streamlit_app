...
if st.session_state.page == 'home':
    ...
    user_hour = st.slider("", 0, 23, value=12)

    if st.button("🚀 Get Recommendation"):
        result = recommend(day, prefer=preference)
        
        # ✅ Best time message كبير وبولد
        st.markdown(f"<h2 style='color:white; font-weight:bold;'>Best time is {int(result['hour'])}:00 🕒</h2>", unsafe_allow_html=True)

        col1, col2 = st.columns(2)
        with col1:
            st.metric(label="🚗 Occupancy", value=f"{round(result['occupancy'],2)}")
        with col2:
            st.metric(label="💰 Price", value=f"{round(result['price'],2)}")
...
st.markdown("""
    ...
    /* Slider Track & Handle */
    div.stSlider>div>div>div>div>div>div>div>div>input[type="range"] {
        accent-color: #1e90ff !important;  /* لون الشريط الأزرق */
    }
    /* Buttons */
    .stButton>button {
